...
const NewProductForm = ({ categorys }) => {
  ...
  const onSubmit = async (data) => {
    ...
    try {
      dispatch(AddProduct({ ...data, img, description: value }, navigate));
  ...
  return (
    <FormProvider methods={methods} onSubmit={handleSubmit(onSubmit)}>
      ...
      >
        ...
        >
          <RHFTextField name="name" label="Tên San pham" />
          <RHFTextField name="price" label="Gia" />
          <RHFTextField name="quantity" label="So luong" />
          <RHFTextField name="mass" label="Khoi luong" />
        </Stack>
        <Stack
          spacing={5}
          sx={{ width: "100%" }}
