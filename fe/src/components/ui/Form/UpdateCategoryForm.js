...
const UpdateCategoryForm = ({ name, description, image, id }) => {
  ...
  const onSubmit = async (data) => {
    ...
    try {
      dispatch(
        UpdateCategory({ ...data, img, id, description: value }, navigate)
      );
    } catch (error) {
      console.log(error);
      reset();
  ...
  return (
    <FormProvider methods={methods} onSubmit={handleSubmit(onSubmit)}>
      <Stack sx={{ width: "60%" }} alignItems={"center"} spacing={3}>
        ...
        <RHFTextField name="name" defaultValue={name} label="Tên Danh Mục" />
        {/* <RHFTextField
          name="description"
